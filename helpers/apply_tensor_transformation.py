...
from helpers.set_shapes import set_shapes
...
def applyTensorTransformationForX_VAL(
    ...
    Y_Val = dataset.map(lambda _,y: y)
    Y_Val_numpy_iterator = [i for i in Y_Val.as_numpy_iterator()]
    
    Y_Val = concatenate(Y_Val_numpy_iterator, axis = 0) # type: ignore
